...
#####
# set python path from R
# library(reticulate)
path_to_python <- "C:/ProgramData/Anaconda3"#/python.exe"
...
umap.data <- umap.data[keep,5:(ncol(umap.data))]
...
combinations = do.call(data.table::CJ, replicate(ncol(umap.data), 0:1, FALSE))
# umap.umap = umap(umap.data)
# head(umap.umap$layout, 3)
combi_sum = rowSums(combinations)
combinations = combinations[combi_sum>=4,]

for (combi_row in 2:nrow(combinations)){

  # iris.data = iris[, grep("Sepal|Petal", colnames(iris))]
  # iris.labels = iris[, "Species"]
  data_subset = umap.data[,which(combinations[combi_row,]==1)]
  umap.umap = umap(data_subset, n_components = 2)
  ...
  fig <- plot_ly(final, x = ~X1, y = ~X2, color = umap.labels,
    ...
    layout(
      title = paste(colnames(data_subset),collapse=" "),
      plot_bgcolor = "#e5ecf6",
  ...
  # fig
  save_image(fig, file = paste0("C:/Users/kdh10kg/OneDrive - The Royal Botanic Gardens, Kew/darkspots/UMAP_test/plots/combination_",combi_row,".png"))
}
