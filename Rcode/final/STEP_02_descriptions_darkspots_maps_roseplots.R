...
data$SR_unknown_sd[which(is.na(data$SR_unknown_sd))] = 1

map <- ggplot() +
  geom_sf(data = data, mapping = aes(fill = (SR_unknown_sd)),
          color = aes(fill = (SR_unknown_sd)),#NA,
          size = 0.4, show.legend = FALSE) +
  geom_sf() +  #+
  geom_point( data= data,
              aes(color =  (SR_unknown_sd),  #fill = bi_class,
                  geometry = geometry),
              size = 2,
              stat = "sf_coordinates"
  ) +
...
legend <- cowplot::get_legend(ggplot() +
                                ...
                                scale_color_gradientn(colours=(RColorBrewer::brewer.pal(7, "Greys")),
                                                      ...
                                                      labels = c("0","1","2","3","4","5","6",">7"))+#[4:7]))+
                                guides(color = "none",
                                       fill=guide_legend(title="darkspot index sd"),
                                       override.aes = list(size = 0.5)) +
                                bi_theme() +
                                geom_path(data = grid.DT[(long %in% c(-180,180) & region == "NS")
                                                         |(long %in% c(-180,180) & lat %in% c(-90,90)
...
finalPlot <- ggdraw() +
  draw_plot(map, 0, 0, 1, 1) +
  draw_plot(legend, 0.8, .6, 0.24, 0.24)
...
data$SR_unknown_sd_sc[which(is.na(data$SR_unknown_sd_sc))] = 1


map <- ggplot() +
  geom_sf(data = data, mapping = aes(fill = (SR_unknown_sd_sc)),
          color = aes(fill = (SR_unknown_sd_sc)),#NA,
          size = 0.4, show.legend = FALSE) +
  geom_sf() +  #+
  geom_point( data= data,
              aes(color =  (SR_unknown_sd_sc),  #fill = bi_class,
                  geometry = geometry),
              size = 2,
              stat = "sf_coordinates"
  ) +
...
legend <- cowplot::get_legend(ggplot() +
                                ...
                                scale_color_gradientn(colours=(RColorBrewer::brewer.pal(7, "Greys")),
                                                      ...
                                                      labels = c("0","0.2","0.4","0.6","0.8",">1"))+#[4:7]))+
                                guides(color = "none",
                                       fill=guide_legend(title="scaled darkspot index sd"),
                                       override.aes = list(size = 0.5)) +
                                bi_theme() +
                                geom_path(data = grid.DT[(long %in% c(-180,180) & region == "NS")
                                                         |(long %in% c(-180,180) & lat %in% c(-90,90)
